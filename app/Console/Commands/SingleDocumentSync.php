...
class SingleDocumentSync extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'siavcom:single-document-sync {conn_name} {doc_num}';
     ...
     * @var string
     */
    protected $description = 'Siavcom Single Document/PCT synchronization.';
    ...
    {
        $conn_name = $this->argument('conn_name');
        $doc_num = $this->argument('doc_num');
        $this->main($conn_name, $doc_num);
    }

    private function main($conn_name, $doc_num)
    {
        $sync_connections = DB::table('sync_connections')->where('name', '=', $conn_name)->first();
        $this->connectAndSync($sync_connections, $doc_num);
    }

    /*Connects and sync pcts from all of the siavcom connections*/
    private function connectAndSync(\stdClass $conn, $doc_num)
    {
