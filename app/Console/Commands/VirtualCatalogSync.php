...
class VirtualCatalogSync extends Command
{
    ...
    {
      $virtual_cat_sets = DB::connection($this->virtual_cat_conn_name)
        ->table('cotizacion')->get(); //->where('rfq', '!=', null)
      ...
      private function createDocumentSupply(\stdClass $virtual_cat_set, Document $document)
      {
        ...
        if(!empty($proveedor)) {
         ...
         $this->attachSupplierManufacturers($supplier);
     ...
     private function attachSupplierManufacturers(Supplier $supplier)
     {
      $siavcom_manufacturers = DB::connection($this->virtual_cat_conn_name)->table('producto_proveedor')
